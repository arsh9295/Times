							...
							if filemoddate==currentsystime:
								...
								if script_name == "avgapplyforfa":
									if (time.strftime('%H')=='23'):
										...
										whiteinfine()
								elif script_name == "TotalJobReport":
									if (time.strftime('%H')=='04'):
										subprocess.call(['cd /opt/JOBAlert_New/;'+line], shell=True)
										subprocess.call(["sed -i '/"+modfile+"/d' /opt/JOBAlert_New/utk/files/logfille"], shell=True)
                                                                                logfile_for_process.write(time.strftime('%d-%m-%y %H:%M')+" Process "+modfile+" Started\n")
										whiteinfine()
								elif script_name == "TotalJob":
									if (time.strftime('%H')=='02'):
										subprocess.call(['cd /opt/JOBAlert_New/;'+line], shell=True)
                                                                                subprocess.call(["sed -i '/"+modfile+"/d' /opt/JOBAlert_New/utk/files/logfille"], shell=True)
                                                                                logfile_for_process.write(time.strftime('%d-%m-%y %H:%M')+" Process  "+modfile+" Started\n")
                                                                                whiteinfine()
								elif any(c in script_name for c in ("walkinreminder2days", "walkinreminder1day")):
									if (time.strftime('%H')=='08'):
										subprocess.call(['cd /opt/JOBAlert_New/;'+line], shell=True)
                                                                                subprocess.call(["sed -i '/"+modfile+"/d' /opt/JOBAlert_New/utk/files/logfille"], shell=True)
                                                                                logfile_for_process.write(time.strftime('%d-%m-%y %H:%M')+" Process  "+modfile+" Started\n")
                                                                                whiteinfine()
								elif script_name == "walkinreminder3hours":
									if int(time.strftime('%H') in range(06, 19)):
										subprocess.call(['cd /opt/JOBAlert_New/;'+line], shell=True)
                                                                                subprocess.call(["sed -i '/"+modfile+"/d' /opt/JOBAlert_New/utk/files/logfille"], shell=True)
                                                                                logfile_for_process.write(time.strftime('%d-%m-%y %H:%M')+" Process  "+modfile+" Started\n")
                                                                                whiteinfine()
                                                                elif script_name == "WalkInJob":
                                                                        if int(time.strftime('%H') in range(00, 23, 2)):
                                                                                subprocess.call(['cd /opt/JOBAlert_New/;'+line], shell=True)
                                                                                subprocess.call(["sed -i '/"+modfile+"/d' /opt/JOBAlert_New/utk/files/logfille"], shell=True)
                                                                                logfile_for_process.write(time.strftime('%d-%m-%y %H:%M')+" Process  "+modfile+" Started\n")
                                                                                whiteinfine()
