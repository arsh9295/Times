					...
					if timedifferenc > 1020:
						...
						if 'Process Completed Successfully' in pidp:
							...
							if filemoddate==currentsystime:
								...
								if any(c in script_name for c in ("Endorsement", "savedSearch")):
									if (time.strftime('%H')=='09' and int(time.strftime('%M')) in range(01, 12)):
										...
										whiteinfine()
								elif any(c in script_name for c in ("hireInstaSms","rewProgram","freshness", "markedspam")):
									if (time.strftime('%H')=='02'):
										subprocess.call(['cd /opt/JOBAlert_New/;'+line], shell=True)
										subprocess.call(["sed -i '/"+modfile+"/d' /opt/JOBAlert_New/utk/files/logfille"], shell=True)
                                                                                logfile_for_process.write(time.strftime('%d-%m-%y %H:%M')+" Process "+modfile+" Started\n")
										whiteinfine()
								elif any(c in script_name for c in ("ActivityMailer", "JobMatchingProfileMailer")):
									if (time.strftime('%H')=='06' and (int(time.strftime('%M')) in range(50, 59))):
										subprocess.call(['cd /opt/JOBAlert_New/;'+line], shell=True)
                                                                                subprocess.call(["sed -i '/"+modfile+"/d' /opt/JOBAlert_New/utk/files/logfille"], shell=True)
                                                                                logfile_for_process.write(time.strftime('%d-%m-%y %H:%M')+" Process  "+modfile+" Started\n")
                                                                                whiteinfine()
								elif script_name == "unsubtjapp":
									if (time.strftime('%H')=='07'):
										subprocess.call(['cd /opt/JOBAlert_New/;'+line], shell=True)
                                                                                subprocess.call(["sed -i '/"+modfile+"/d' /opt/JOBAlert_New/utk/files/logfille"], shell=True)
                                                                                logfile_for_process.write(time.strftime('%d-%m-%y %H:%M')+" Process  "+modfile+" Started\n")
                                                                                whiteinfine()
								elif script_name == "EmailedJobAlertMailer":
									if (time.strftime('%H')=='01' and datetime.datetime.today().weekday()=='0'):
										subprocess.call(['cd /opt/JOBAlert_New/;'+line], shell=True)
                                                                                subprocess.call(["sed -i '/"+modfile+"/d' /opt/JOBAlert_New/utk/files/logfille"], shell=True)
                                                                                logfile_for_process.write(time.strftime('%d-%m-%y %H:%M')+" Process  "+modfile+" Started\n")
                                                                                whiteinfine()
                                                                elif script_name == "markedtimesjobspam":
                                                                        if (time.strftime('%H')=='00' and (int(time.strftime('%M') in range(30, 40)))):
                                                                                subprocess.call(['cd /opt/JOBAlert_New/;'+line], shell=True)
                                                                                subprocess.call(["sed -i '/"+modfile+"/d' /opt/JOBAlert_New/utk/files/logfille"], shell=True)
                                                                                logfile_for_process.write(time.strftime('%d-%m-%y %H:%M')+" Process  "+modfile+" Started\n")
                                                                                whiteinfine()
					...
					if timedifferenc > 1020:
						...
						if 'Process Completed Successfully' in pidp:
							...
							if currentsystime == datelog[modfile]:
								...
								if modfile == "smscycle3":
									if (int(time.strftime('%H')) in range(00, 23)) and (int(time.strftime('%H')) in range(30, 60)):
										...
										subprocess.call(['cd /opt/JOBAlert_New/; nohup '+script_name+' '+modfile+' '+currentsystime+' &'], shell=True)
                                                                                logfile_for_process.write(time.strftime('%d-%m-%y %H:%M')+" Process "+modfile+" Started\n")
										whiteinfine()
								elif modfile == "Activity":
									if (int(time.strftime('%H')) in range(06, 23)):
										subprocess.call(["sed -i '/\<"+modfile+"\>/d' /opt/JOBAlert_New/utk/files/cycle.txt"], shell=True)
                                                                                subprocess.call(["sed -i '/"+modfile+"/d' /opt/JOBAlert_New/utk/files/logfille"], shell=True)
										subprocess.call(['cd /opt/JOBAlert_New/; nohup '+script_name+' '+modfile+' '+currentsystime+' &'], shell=True)
                                                                                logfile_for_process.write(time.strftime('%d-%m-%y %H:%M')+" Process "+modfile+" Started\n")
